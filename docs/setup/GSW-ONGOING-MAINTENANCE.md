...
| 개발 브랜치 | `develop` |
| 배포 후 스모크 | 로그인 → `#/portal` VAN/QQ 선택, VAN 정비지침 3차종, QQ `#/qq/shop` SM, `#/dtc`·공지/커뮤니티, PWA 홈 화면 |
...
| 항목 | 관리 |
|------|------|
| GSW ↔ LMS 브릿지 | [`GSW-LMS-BRIDGE.md`](GSW-LMS-BRIDGE.md) — `GSW_BRIDGE_SECRET` 포털·LMS 동일, 「교육 센터」→ `/auth/gsw` HMAC 토큰 redirect |
...
| VAN / QQ 포털 분리 | `#/portal` 계통 선택 · VAN `#/home` `#/shop`(2VAN/4VAN/Cargo) · QQ `#/qq/home` `#/qq/shop`(SM 고정). QQ 스플래시 `assets/qq/splash.jpg`. 모듈 [`js/productLine.js`](../../js/productLine.js), [`js/portalPicker.js`](../../js/portalPicker.js) |
...
| 저장 항목 | 이벤트 유형(`session_start`, `route_view`, `manual_open`, `manual_download`, `dtc_select`, `admin_user_update`, `admin_user_create`, `admin_user_delete`, `warranty_master_export` 등), 카테고리·리소스 키 요약, JSON payload(버킷·파일명·건수 등 비식별 범위) |
| 행위자 | 트리거로 `actor_user_id = auth.uid()` 고정(클라이언트 입력 무시) |
| 관리 화면 | **이용 로그** 탭은 **수퍼바이저(EK0V029)만** 표시(기본 최근 7일, 최대 500건). payload는 DB에만 저장·화면 미표시 |
| 수퍼바이저 | **EK0V029** 전용. **ID·이메일·이름·소속·역할·등급** 수정, **계정 생성·삭제**(각각 본인 이메일 OTP 재인증 후, `035` RPC) |
| 웹 사용자 관리 | **관리자(role)** 는 **등급(blue/silver/black)만** 변경·이용 로그 탭 없음. **수퍼바이저** 는 ID·이메일·역할·등급·이용 로그 |
