...
if (window.__APP_INIT__) {
    ...
    (function(){
        ...
        // 정비지침서 매핑 병합 (별도 파일에서 로드된 경우)
        if (typeof maintenanceManualMapping !== 'undefined') {
            Object.assign(PDF_MAPPING, maintenanceManualMapping);
        }
        
        // ETM 매핑 병합 (별도 파일에서 로드된 경우)
        if (typeof etmMapping !== 'undefined') {
        ...
        function getTreeDataByTitle(title) {
            const dataMap = {
                '정비지침서': typeof maintenanceManualTreeData !== 'undefined' 
                ...
                '전장회로도': typeof etmTreeData !== 'undefined' 
                    ? etmTreeData 
                    : [
                        {
                            id: 'etm-1',
    ...
    if (confirm('로그아웃 하시겠습니까?')) {
                    ...
                    authContainer.innerHTML = `
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs ${timeColor} font-mono font-semibold" id="session-timer">${timeStr}</span>
                            </div>
                            <button onclick="refreshSessionManually()" class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                                세션 갱신
                            </button>
                            <button onclick="handleLogout()" class="text-sm text-red-600 hover:text-red-800">로그아웃</button>
                        </div>
                    `;
                } else {
                    stopSessionTimer();
        ...
        async function setupWatermark() {
            ...
            try {
                ...
                let watermarkText = '';
                if (userInfo) {
                    watermarkText = `${userInfo.affiliation || '사용자'} - ${userInfo.name || '사용자'} - ${new Date().toLocaleDateString()}`;
                } else {
                    watermarkText = 'CONFIDENTIAL - ' + new Date().toLocaleDateString();
                }
...
async function createWatermarkImage(text, username = null, userInfo = null) {
    return new Promise((resolve, reject) => {
        ...
        font.load().then((loadedFont) => {
            ...
            // 한글 워터마크 텍스트 생성
            if (userInfo && username && username !== 'USER') {
                // 한글 워터마크: 이름 - 소속 - 날짜 시간
                ...
                // 원본 텍스트 사용
                console.log('📝 Using provided text:', finalText);
            }
...
async function getWatermarkedFileUrl(bucketName, fileName, pageRange = null) {
    ...
    // 한글 워터마크 텍스트 생성
    const watermarkText = `${userInfo.name || username} - ${userInfo.affiliation || '소속'} - ${dateString}`;
    
    console.log('📝 Korean watermark text:', watermarkText);
    ...
    try {
        ...
        const fallbackText = `${username} - ${englishDateStr} ${englishTimeStr}`;
    ...
    try {
        ...
        async function initApp() {
            try {
                ...
                hideSplashScreen();
                showToast('앱 로딩 중 오류가 발생했습니다.', 'error');
            }
        }
