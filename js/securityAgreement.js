...
window.securityAgreement = {
    ...
    async saveAgreement(company, name) {
        try {
            ...
            // auth_user_id로 업데이트 시도
            let { error: updateError } = await window.supabaseClient
                .from('users')
                .update(updateData)
                .eq('auth_user_id', session.user.id);

            // auth_user_id로 업데이트 실패 시 이메일로 시도
            if (updateError && session.user.email) {
                ...
                if (emailError) {
                    console.error('❌ 보안서약서 동의 저장 실패:', emailError);
                    return false;
                }
            } else if (updateError) {
                console.error('❌ 보안서약서 동의 저장 실패:', updateError);
                return false;
            }
